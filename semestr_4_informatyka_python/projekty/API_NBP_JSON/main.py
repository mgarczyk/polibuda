...
def save_data_to_JSON_day(currency, date):  # funckja zapisuje dane z jednego dnia do pliku
...
def save_data_to_JSON_interval(currency, date_1, date_2): #funkcja zapisuje dane z przedziału czsu do pliku
...
def read_data_from_JSON_day(currency, date):
...
def read_data_from_JSON_interval(currency, date_1, date_2):
...
def get_from_NBP_to_JSON(): # pobieranie danych z NBP do naszego pliku
    ...
    if choose_date == "/day":
        ...
        save_data_to_JSON_day(currency, date)  #funkcja zapisze dane z danego dnia do pliku
        try:
            read_data_from_JSON_day(currency, date) #odczyt danych za pomocą funkcji
        ...
        save_data_to_JSON_interval(currency, date_1, date_2)
        try:
            read_data_from_JSON_interval(currency, date_1, date_2)  #odczyt danych za pomocą funkcji
        except json.decoder.JSONDecodeError:   #gdyby user podał datę np. z przyszłośći to wyskoczy  error, który obsłuigujemy
...
def read_from_JSON(): #wyświetlanie odpowiednich informacji
   print("Na przyśłość, narazie odczytuje z tego co pobiera i tyle") #TODO może się przydać ta funkcja kiedyś do odczytu z JSONA
