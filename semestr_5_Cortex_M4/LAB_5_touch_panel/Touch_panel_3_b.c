...
int32_t TouchCallbackDisplayInCentimeters(uint32_t ulMessage, int32_t lX, int32_t lY)
{

    char fullText[30];
    GrContextForegroundSet(&sContext, ClrWhite);;
    sprintf(fullText, "Pressed %f %f", (i32CenterX*5.76/320), (i32CenterY*4.32/240)); //mnożymy wartość piksela, przez współaczynnik długość/ilość pikseli - w danym w wymiarze, dane z parametrów wyświetlacza
    GrStringDrawCentered(&sContext, fullText, -1, 50, 50, 0);
    GrContextForegroundSet(&sContext, ClrGreen);
    i32CenterX = lX;
    i32CenterY = lY;
    printf(fullText, "Pressed %f %f", (i32CenterX*5.76/320), (i32CenterY*4.32/240));
    GrStringDrawCentered(&sContext, fullText, -1, 50, 50, 0);
    return 0;
}
...
//                       SYSCTL_OSC_MAIN);
    ...
    TouchScreenCallbackSet(TouchCallbackDisplayInCentimeters);
