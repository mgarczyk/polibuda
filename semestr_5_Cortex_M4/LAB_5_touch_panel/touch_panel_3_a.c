...
int32_t TouchCallbackRelativeToMiddleOFScreen(uint32_t ulMessage, int32_t lX, int32_t lY)
{

    char fullText[30];
    GrContextForegroundSet(&sContext, ClrWhite);
    sprintf(fullText,"Pressed %d %d", i32CenterX - 160, i32CenterY - 120); //odejmujemy od odpowiednich współrzędnych, odpowiadające im współrzędne środka ekranu
    GrStringDrawCentered(&sContext, fullText, -1, 50, 50, 0);
    GrContextForegroundSet(&sContext, ClrGreen);
    sprintf(fullText,"Pressed %d %d", lX - 160, lY - 120); //odejmujemy od odpowiednich współrzędnych, odpowiadające im współrzędne środka ekranu
    i32CenterX = lX;
    i32CenterY = lY;
    GrStringDrawCentered(&sContext, fullText, -1, 50, 50, 0);
    return 0;
...
//                       SYSCTL_OSC_MAIN);
    ...
    TouchScreenCallbackSet(TouchCallbackRelativeToMiddleOFScreen);
